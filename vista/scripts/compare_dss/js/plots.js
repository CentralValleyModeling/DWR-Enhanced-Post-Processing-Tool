...
Plots.prototype.time_series_plot = function (div_id, data, diff, sdate, edate, bars) {
    ...
    if (bars) {
        ...
        vis.add(pv.Bar).data(bars)
            .bottom(h2).height(h)
            .left(function (d) {
                return truncate(x(d.start), x.range());
            })
            .width(function (d) {
                return truncate(x(d.end), x.range()) - truncate(x(d.start), x.range());
            })
    ...
    vis.add(pv.Line)
        ...
        .visible(function (d) {
            if (d.x >= sdate && d.x <= edate) {
                return isNaN(d.y1) ? false : true
            }
        })
        .left(function (d) {
            if (d.x >= sdate && d.x <= edate) return x(d.x)
        })
        .bottom(function (d) {
            if (d.x >= sdate && d.x <= edate) return y(d.y1)
        })
    ...
    vis.add(pv.Line)
        ...
        .segmented(true)
        .visible(function (d) {
            if (d.x >= sdate && d.x <= edate) return isNaN(d.y2) ? false : true
        })
        .left(function (d) {
            if (d.x >= sdate && d.x <= edate) return x(d.x)
        })
        .bottom(function (d) {
            if (d.x >= sdate && d.x <= edate) return y(d.y2)
        })
    ...
    if (diff) {
        vis.add(pv.Line)
            ...
            .segmented(true)
            .visible(function (d) {
                if (d.x >= sdate && d.x <= edate) return isNaN(diff_data[this.index]) ? false : true
            })
            .left(function (d) {
                if (d.x >= sdate && d.x <= edate) return x(d.x)
            })
            .bottom(function (d) {
                if (d.x >= sdate && d.x <= edate) return yd(diff_data[this.index]);
            })
...
Plots.prototype.multi_time_series_plot = function (div_id, data, diff, sdate, edate, bars) {
    ...
    if (y0 == 1) {
        ts[i] = {"type": "dot", "series": "y0", "name": data.series_names[0]};
        i++;
    }
    if (y1 == 1 && cou1 == 0) {
        ts[i] = {"type": "line", "series": "y1", "name": data.series_names[1]};
        cou1++;
        i++;
    }
    if (y2 == 1 && cou2 == 0) {
        ts[i] = {"type": "line", "series": "y2", "name": data.series_names[2]};
        cou2++;
        i++;
    }
    if (sdate == null) {
    ...
    if (diff) {
        ...
        if (y0 == 1 && y1 == 1 && y2 == 1) { // case 5
            ...
            if (diff_data[0].sumabs() == 0 && diff_data[1].sumabs() == 0) yd = pv.Scale.linear([-0.01, 0.01]).range(0, h2 - 50); /* add this line to fix the problem of x axis for zero array */
                ...
                yd = pv.Scale.linear([diff_concat.min(), diff_concat.max()]).range(0, h2 - 50);
            }
        } else {
    ...
    for (i = 0; i < ts.length; i++) {
        ...
        if (ts[i].type == 'dot') {
            vis.add(pv.Dot)
                ...
                .visible(function (d) {
                    if (d.x >= sdate && d.x <= edate) return isNaN(d.y0) ? false : true;
                })
                .left(function (d) {
                    if (d.x >= sdate && d.x <= edate) return x(d.x)
                })
                .bottom(function (d) {
                    if (d.x >= sdate && d.x <= edate) return y(d.y0)
                })
    ...
    if (diff) {
        if (y0 == 1 && y1 == 1 && y2 == 1) {
            vis.add(pv.Line)
                ...
                .segmented(true)
                .visible(function (d) {
                    if (d.x >= sdate && d.x <= edate) return isNaN(diff_data[1][this.index]) ? false : true
                })
                .left(function (d) {
                    if (d.x >= sdate && d.x <= edate) return x(d.x)
                })
                .bottom(function (d) {
                    if (d.x >= sdate && d.x <= edate) return yd(diff_data[1][this.index]);
                })
        ...
        vis.add(pv.Line)
            ...
            .segmented(true)
            .visible(function (d) {
                if (d.x >= sdate && d.x <= edate) return isNaN(diff_data[0][this.index]) ? false : true
            })
            .left(function (d) {
                if (d.x >= sdate && d.x <= edate) return x(d.x)
            })
            .bottom(function (d) {
                if (d.x >= sdate && d.x <= edate) return yd(diff_data[0][this.index]);
            })
...
Plots.prototype.exceedance_plot = function (div_id, data) {
    ...
    var w = this.PLOT_WIDTH,
        ...
        y = pv.Scale.linear(data.values, function (d) {
            return Math.min(d.y1, d.y2)
        }, function (d) {
            return Math.max(d.y1, d.y2)
        }).range(0, h);
...
Plots.prototype.time_series_derivative_plot = function (div_id, data) {
    ...
    var w = this.PLOT_WIDTH,
        ...
        x = pv.Scale.linear(data.values,
            function (d) {
                return Math.min(d.y1) - 0.05 * Math.abs(d.y1)
            },
            function (d) {
                return Math.max(d.y1) + 0.05 * Math.abs(d.y1)
            })
            .range(0, w),
        y = pv.Scale.linear(data.values,
            function (d) {
                return Math.min(d.y2) - 0.05 * Math.abs(d.y2)
            },
            function (d) {
                return Math.max(d.y2) + 0.05 * Math.abs(d.y2)
            })
            .range(0, h);

    /* Interaction state. Focus scales will have domain set on-render. */
    var h2 = 30;
    var c = pv.Scale.linear(data.values, function (d) {
        return d.y3
    }).range("red", "yellow");
    ...
    vis.add(pv.Bar)
        ...
        .textMargin(20)
        .visible(function (d) {
            return this.index == 0 || this.index == 29;
        })
        .text(function (d) {
            if (this.index == 0) {
...
Plots.prototype.xsection_editor = function (div_id, xsection_points, profile_points, points) {
    ...
    var xsection = xsection_points.map(function (d) {
    ...
    vis.add(pv.Dot)
        ...
        .strokeStyle(function () {
            return i == this.index ? "#ff7f0e" : "#1f77b4"
        })
        .fillStyle(function () {
        ...
        .event("dragstart", function () {
            return (i = this.index, this)
        })
        .event("dragend", function (d) {
            i = this.index;
            var xp = xsection_points[i];
            var p = xsection[i];
            xp.x = x.invert(p.x);
            xp.y = y.invert(p.y);
        })
