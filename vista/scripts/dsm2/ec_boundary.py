...
def getMTZCoef():
    return [0.00276159212914926, -6.070476764627e-005, 0.000151597503356787, -1.0510895394871e-005,
            -2.8338337830349e-006, 4.9586716960501e-005, -8.7552561972579e-005, 7.2094495694158e-005,
            -5.1752915755519e-005]
...
def ECEst(stage, ndo, beta, npow1, npow2, g0=None, zrms=None, c=getMTZCoef()):
    ...
    while (not giter.atEnd()):
        ...
        ecfrac = c[0] + gval * c[1] + 1.1 * gval ** npow1 * (c[2] * zarr[i + 72] + c[3] * zarr[i + 60]
                                                             ...
                                                             + c[6] * +zarr[i + 24] + c[7] * zarr[i + 12] + c[8] * zarr[
                                                                 i]) - 0.1
        # +  c[9]*zrmsval
