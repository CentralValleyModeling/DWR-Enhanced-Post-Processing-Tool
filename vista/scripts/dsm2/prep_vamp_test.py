...
class VampTestCase(unittest.TestCase):
    ...
    def test_calculate_exports_average_above_limit(self):
        ...
        for i in range(0, len(export_limit_array)):
            self.assertEqual(export_limit[i],
                             round(export_limit_array[i], 0)), 'calculate_exports function returned wrong value'
    ...
    def test_project_export_only_apr_may(self):
        ...
        swp_out_from_function, cvp_out_from_function = prep_vamp.project_export_limits(pulse_limit, ei_ratio,
                                                                                       delta_inflow)
        swp_array = swp_out_from_function.getYArray()
        cvp_array = cvp_out_from_function.getYArray()
        for i in range(0, len(cvp_array)):
            self.assertEqual(cvp_out[i],
                             round(cvp_array[i], 0)), 'project_export function returned wrong cvp_limit value'
            self.assertEqual(swp_out[i],
                             round(swp_array[i], 0)), 'project_export function returned wrong swp_limit value'
    ...
    def test_compare_with_spreadsheet(self):
        ...
        if cvp_start_year_ss < cvp_start_year_script:
            raise ValueError(
                "start time of cvp exports from the spreadsheet is ahead ofthe start time from the script.")
        ...
        for i in range(len(cvp_pre_vamp_script)):
            f_out.write("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n" % (year, cvp_pre_vamp_ss[i], cvp_pre_vamp_script[i], \
                                                                  cvp_vamp_apr_ss[i], cvp_vamp_apr_script[i],
                                                                  cvp_vamp_may_ss[i], cvp_vamp_may_script[i], \
        ...
        for i in range(len(cvp_pre_vamp_script)):
            f_out.write("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n" % (year, swp_pre_vamp_ss[i], swp_pre_vamp_script[i], \
                                                                  swp_vamp_apr_ss[i], swp_vamp_apr_script[i],
                                                                  swp_vamp_may_ss[i], swp_vamp_may_script[i], \
        ...
        for export in exports_no_zeros:
            if (i != 1) and (i != (mid_day + 1)):
                if round(pre_val, 0) != round(export, 0):
                    raise ValueError(
                        "the value should not be change during this period. previous value: (%d), current value: (%d)" \
                        % (pre_val, export))
