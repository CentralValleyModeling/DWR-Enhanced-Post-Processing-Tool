...
from vista.set import Constants, DataType, FlagUtils, \
    IrregularTimeSeries, MovingAverageProxy, ProxyFactory, \
    Stats, TimeSeriesMath
...
def nearest_index(ds, timeinst, ndxHint=0):
    ...
    if ndxHint >= 0 and ndxHint < len(ds):
        ...
        if long(ds.getElementAt(ndxHint).getX()) > tii:
            ndxStart = 0
        else:
            ndxStart = ndxHint
        ndxEnd = len(ds)
...
def per_max_time(ds, interval='1mon'):
    ...
    """
    return TimeSeriesMath.getPeriodMinMax(ds, TimeFactory.getInstance().createTimeInterval(interval),
...
def mov_avg(dsref, backLength, forwardLength):
    ...
    for ndx in range(totLength - 1):
        ...
        if filter.isAcceptable(el1):
            vecY[ndx + 1] = el1.getY()
        else:
            vecY[ndx + 1] = smallNumber
    for ndx in range(ds.size()):
        ...
        if ndx < backLength or ndx >= (ds.size() - forwardLength):
            ...
            if filter.isAcceptable(el2):
                vecY.append(el2.getY())
            else:
                vecY.append(smallNumber)
            vecY.pop(0)
...
from vista.set import DataReference, Pathname, RegularTimeSeries, Constants
