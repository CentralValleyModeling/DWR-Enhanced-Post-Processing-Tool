   ...
   '''
    ...
    for ref in g1:
        ...
        if c_part == None:
            ...
            g3 = findparts(g2, b=path.getPart(Pathname.B_PART), c=path.getPart(Pathname.C_PART),
                           e=path.getPart(Pathname.E_PART), f=path.getPart(Pathname.F_PART))
        else:
            # g3 = findparts(g2,a=a[1],b=a[2],c=c_part)
            g3 = findparts(g2, b=path.getPart(Pathname.B_PART), c=c_part, e=path.getPart(Pathname.E_PART),
                           f=path.getPart(Pathname.F_PART))
        if g3 is not None and (path.getPart(Pathname.C_PART) == c_part or c_part == None) and path.getPart(
                Pathname.E_PART) != 'IR-YEAR':
            ...
            if ref1 is None:
                print 'Error: The time window ' + tw_string + ' is out of range for ' + str(
                    ref.pathname) + '! Please specify the correct time window.'
                break
            elif ref2 is None:
                print 'Error: The time window ' + tw_string + ' is out of range for ' + str(
                    g3[0].pathname) + '! Please specify the correct time window.'
                break
            else:
                ref3 = ref2 - ref1
            try:
    ...
    for mpath, val in metric.iteritems():
        ...
        try:
            if if_id_not_exist(dsm2model.get_id_by_name(pname.lower()), chkarr):
                ...
                f.write(a['channel_id'] + ',' + a['channel_name'].upper() + ',' + str(a['longitude']) + ',' + str(
                    a['latitude']) + ',' + str(val) + '\n')
