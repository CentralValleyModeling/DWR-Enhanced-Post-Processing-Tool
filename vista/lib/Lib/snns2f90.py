...
def dumpf90(outfile, modulename, uline, sline, wline, defsarray, exec_lines):
    ...
    outf.write('contains');
    outf.write(eol)
    outf.write('subroutine ' + modulename + '_initall()');
    outf.write(eol)
    ulen = len(uline)
    clen = 3
    nunit = 0
    ...
    for xdef in defsarray:
        ...
        outf.write(eol)
    # 
    outf.write('end subroutine ' + modulename + '_initall');
    outf.write(eol)
    #
    outf.write('subroutine ' + modulename + '_engine(inarray, outarray, init)');
    outf.write(eol)
    outf.write('  real , dimension(:), intent(in) :: inarray');
    outf.write(eol)
    outf.write('  real , dimension(:), intent(inout) :: outarray');
    outf.write(eol)
    outf.write('  integer , intent(inout) :: init');
    outf.write(eol)
    outf.write('  integer member,source');
    outf.write(eol)
    #    outf.write('  type(ut) :: unit'); outf.write(eol)
    outf.write('  integer :: unitnr');
    outf.write(eol)
    outf.write('  real sum');
    outf.write(eol)
    in_source = 0
    ...
    for line in exec_lines:
        if find(line, "for") >= 0 and find(line, "member") >= 0:  # for(member = init_val
            ...
            outf.write("do " + loop_var + "= " + init_val + ", " + final_val);
            outf.write(eol)
    ...
    #
    #
    outf.write('end subroutine ' + modulename + '_engine');
    outf.write(eol)
    #
    outf.write('end module ' + modulename);
    outf.write(eol)
    outf.close()
