...
# basic java stuff
from java.io import BufferedOutputStream, FileInputStream
...
from vista.graph import TextLine, CurveAttr
...
class GraphData:
    ...
    def saveimg(self, dir_name="", type="gif", prefix=""):
        ...
        self._dummy_fr = JFrame();
        self._dummy_fr.setVisible(1);
        self._dummy_fr.setVisible(0)
    ...
    def _graph(self, plots, gtitle, plot_rows=3, plot_cols=1, display=1):
        ...
        """
        # create Graph
        from vista.graph import GEBorderLayout
        ppg = plot_rows
        mp = None
        count = 0
    ...
    def _plot(self, crvAttrs, simple_plot=0):
        ...
        if txts != None and len(txts) >= 0:
            ...
            for i in range(len(txts)):
                ...
                dynamic_eval(sizes[i], globals(), ldict), dynamic_eval(xs[i], globals(), ldict), dynamic_eval(ys[i],
                                                                                                              globals(),
                                                                                                              ldict)
