...
public class TimeseriesChartPane extends JFXPanel
{
	private static final Logger LOGGER = Logger.getLogger(TimeseriesChartPane.class.getName());
	private JavaFxChartsPane _javaFxChartsPane;


	public TimeseriesChartPane(GUILinksAllModelsBO guiLink, List<EpptScenarioRun> scenarioRuns, PlotConfigurationState.ComparisonType comparisonType,
							   LocalDate start, LocalDate end, boolean taf)
	{
		...
		if(comparisonType == PlotConfigurationState.ComparisonType.DIFF)
		{
			...
			epptReportingComputedSet = EpptReportingComputedSet.computeDiffForMetrics(guiLink,
					...
					new MonthPeriod(java.time.Month.OCTOBER, Month.SEPTEMBER),
					start,
					end,
					taf,
					scenarioRuns.get(0),
		...
		Platform.runLater(() ->
		{
			_javaFxChartsPane = new JavaFxChartsPane(Paths.get(Constant.CONFIG_DIR).resolve("plots").resolve("timeseries.htm"),
