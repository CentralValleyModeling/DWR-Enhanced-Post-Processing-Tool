...
public class AllExceedancePane extends JFXPanel
{
	private static final Logger LOGGER = Logger.getLogger(AllExceedancePane.class.getName());
	private JavaFxChartsPane _javaFxChartsPane;

	public AllExceedancePane(GUILinksAllModelsBO guiLink, List<EpptScenarioRun> scenarioRuns, PlotConfigurationState.ComparisonType comparisonType,
							 LocalDate start, LocalDate end, boolean taf)
	{
		...
		Platform.runLater(() ->
		{
			_javaFxChartsPane = new JavaFxChartsPane(Paths.get(Constant.CONFIG_DIR).resolve("plots").resolve("exceedance_discrete.htm"),
