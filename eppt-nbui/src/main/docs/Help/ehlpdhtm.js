...
var gBsOnLoads = [];	// An array holds all the onload event handler.
var gBsOnClicks = [];	// An array holds all the onClick event handler.
var gBsOnUnLoads = [];	// An array holds all the OnUnLoad event handler.
var gBsOnMouseOvers = [];	// An array holds all the OnMouseOver event handler.
var gBsOnMouseOuts = [];	// An array holds all the OnMouseOut event handler.
...
function IsDirty(nIndex) {
    if (nIndex == -1)
        return true;
    else if (arrayDirty.length > nIndex)
...
function _BSSCPopup(strURL, width, height) {
    ...
    if (DHTMLPopupSupport()) {
        if (!gbAIR && !gbInsideCHM)
            ...
            if (IsDirty(nIndex)) {
                if (gbAIR) {
                    var tempappTimer = setInterval(function () {
                        DelayBSSCPopup_AfterLoad(nIndex, nToken, cuswidth, cusheight)
                    }, 400);
...
function AdjustObjectTag(strRawHTML, nStartPos) {// adjust object tag for related topics HTML control, because innerHTML misses out the item settings
...
function kadovAdjustObjectTag(strRawHTML, nStartPos) {// adjust object tag for related topics HTML control, because innerHTML misses out the item settings
...
function InitEffect(element, prop) {
    ...
    for (var i = 0; i < functions.length; i++) {
        ...
        for (var j = 0; j < arrForClickCount.length; j++) {// to locate and get the "clicks=99" settings
        ...
        var args = srcargs;
        if (j < arrForClickCount.length) {// to strip out the "clicks=99" from the arguments string
            args = "";
...
function CCSSP() {
} // constructor of CCSSP class

CCSSP.GetObject = function (obj) {//convert object name string or reference into a valid object reference
...
CCSSP.MoveObjectTo = function (obj, x, y) {//positioning an object at a specific pixel coordinate
    if (gbBsNS4 && !gbBsNS6)
        obj.moveTo(x, y);
    else {
        if (x == '')
    ...
    if (obj.timer == null) {
        if (gbAIR) {
            obj.timer = setInterval(function () {
                CEngine.PerformAnimation(obj.aniIndex)
            }, timeout);
        } else
...
CCSSP.MoveObjectBy = function (obj, dx, dy) {//moveing a object by x and/or y pixel
    if (gbBsNS6) {
...
CCSSP.SetObjectBGColor = function (obj, color) {//set the background color of an object
    if (gbBsNS6)
...
CCSSP.ShowObject = function (obj, bShow) {// set the object to be visible or invisible
    if (gbBsNS6)
...
CCSSP.GetObjectLeft = function (obj) {// retrieve the x coordinate of a posionable object
    if (gbBsNS6)
...
CCSSP.GetObjectTop = function (obj) {// retrieve the y coordinate of a posionable object
    if (gbBsNS6)
...
CCSSP.GetObjectContainLeft = function (obj) {// retrieve the x coordinate of a posionable object relative to it's parent element
...
CCSSP.GetObjectWindowLeft = function (obj) {// retrieve the x coordinate of a posionable object relative to browser window
    if (gbBsNS4 && !gbBsNS6)
        return obj.pageX;
    else {
...
CCSSP.GetObjectContainTop = function (obj) {// retrieve the y coordinate of a posionable object relative to it's parent element
...
CCSSP.GetObjectWindowTop = function (obj) {// retrieve the y coordinate of a posionable object relative to browser window
    if (gbBsNS4 && !gbBsNS6)
        return obj.pageY;
    else {
...
CCSSP.GetObjectHeight = function (obj) {// retrieve the height of a posionable object
    if (gbBsNS4 && !gbBsNS6)
...
CCSSP.GetObjectWidth = function (obj) {// retrieve the width of a posionable object
    if (gbBsNS4 && !gbBsNS6)
...
CCSSP.RegisterEventHandler = function (srcObj, rawEventName, funcHandler) { // to add the "funcHandler" as the "rawEventName" 's handler to the "srcObj" object,the original event handler will be combined
    ...
    if (gbBsMac && gbBsIE4 && !gbBsIE5) {
        if (typeof (srcObj[rawEventName.toLowerCase()]) == "unknown") { //search for <SCRIPT> tag which define the event handler
        ...
        } else if (gbBsIE4) { //search for <SCRIPT> tag which define the event handler
            for (var i = 0; i < document.scripts.length; i++) {
                var script = document.scripts[i];
                if ((script.htmlFor == srcObj.id || script.htmlFor == srcObj) && script.event == rawEventName) {
                    oldHandler = script.innerHTML;
...
CCSSP.GetWindowHeight = function () {// retrieve the height of available content in browser window
    if (gbBsNS4)
...
CCSSP.GetWindowBottom = function () {// retrieve the bottom postion of browser window
    if (gbBsNS4)
...
CCSSP.GetWindowWidth = function () {// retrieve the width of available content in browser window
    if (gbBsNS4)
...
CCSSP.GetWindowRight = function () {// retrieve the right postion of browser window
    if (gbBsNS4)
...
CCSSP.TrimString = function (objString, subtrim) {// to trim the "subtrim" in the beginning and ending of a string object
...
CCSSP.GetEventElement = function (navEventObject) {// to get the element who fired the current event
    if (gbBsNS4)
...
CCSSP.PrepareFilter = function (Obj) {//to prepare for making the filter work
    Obj.style.filter = "";
...
CTrigger.prototype.AddTargetID = function (strTargetID) {// add one target ID string to the objStrTarget
...
CTrigger.prototype.OnTriggerClick = function (event) {// to activate all asociated target
...
function CTarget(TargetElement) {
    ...
    this.objManager = {}; // object: the event manager
}

CTarget.nPageClickCounter = 0;// static class property.

CTarget.prototype.GetAgencyObject = function (str_action_type, action_setting) {// return the action agency ( effect )object's refernece
    switch (str_action_type) {
        case "show":
            return new CAgencyShow(this.eleTarget, true);
        case "hide":
            return new CAgencyShow(this.eleTarget, false);
...
CTarget.prototype.SetEventManager = function (
    one_event_type, str_action_type, action_setting, event_additional) {// to set the event manager with specified action
    if (typeof (one_event_type) != "string" ||
        typeof (str_action_type) != "string" ||
        typeof (action_setting) != "string")
        return false;
    if (typeof (this.objManager[one_event_type]) == "undefined") {
        this.objManager[one_event_type] = {};
    ...
    if (eventAgency != null) {
        ...
        if (one_event_type == "bsscpageclick") {// to deal with the "number of pageclick" stuff
...
CTarget.prototype.OnEvent = function (strBsscEvent, event) {// response to the event ( bssc format )
    if (typeof (this.objManager[strBsscEvent]) == "object") { // to get the event agency from the event manager
        ...
        for (var i = 0; i < eventAgency.length; i++) {
            ...
            if (strBsscEvent == "bsschover" && event.type == "mouseout")
...
CTarget.prototype.SetState = function (strBsscEvent) {
    ...
    if (strBsscEvent == "bsscpageclick") {// we only set the initial state for the minium number of pageclick
...
function CEngine() {
}// all properities are going be "class" properities
...
CEngine.arrAnimation = [];
CEngine.PerformAnimation = function (nIndex) {// animation : update effects function
    CEngine.arrAnimation[nIndex].UpdateEffect();
};

CEngine.AddOneTrigger = function (TriggerID, TriggerElement) {// add one Trigger object into the trigger array
...
CEngine.AddOneTarget = function (TargetID, TargetElement) {// add one Target object into the target array
...
CEngine.SendEventToOneTarget = function (strTargetID, strBsscEvent, event) {// to activate one target object
    if (typeof (CEngine.objTarget[strTargetID]) == "object") {
        if (strBsscEvent == "bssctrigger1" || strBsscEvent == "bssctrigger2") {//now, the "bssctrigger1" and "bssctrigger2" work like a toggle
...
CEngine.SendEventToAllTarget = function (strBsscEvent, event) { //to activate all target associative to the BSSC event
    for (var strTargetID in CEngine.objTarget) //to enumerate all target
        CEngine.SendEventToOneTarget(strTargetID, strBsscEvent, event);
};

CEngine.SetOneTargetInitialState = function (strTargetID) {// only invoked after ALL effects for the target have been set
    if (typeof (CEngine.objTarget[strTargetID]) == "object") {// to get target object
...
CEngine.OnPageClick = function (event) { // to invoke all target's onpageclick handler
    var src = CCSSP.GetEventElement(arguments[0]);
    if (src == null)
        return;

    var objClickedTrigger = null;
    for (var strTriggerID in CEngine.objTrigger) { // to detect which trigger is clicked
...
CEngine.OnMouseOver = function (event) { // to invoke all target's onpageload handler
    var src = CCSSP.GetEventElement(arguments[0]);
    if (src == null)
        return;

    var strHoveredTargetID = null;
    for (var strTargetID in CEngine.objTarget) { // to detect which Target is hovering on
...
CEngine.BuildTargetObject = function (target_ID, event_type, action_type,
                                      action_setting, event_additional) {// to build target object
    ...
    if (arrEvent.length > 1) {// if event is combined, it must be : "bssctrigger1 | bssctrigger2"
...
CEngine.BuildTriggerObject = function (trigger_ID, target_ID) {// to build the trigger object
    var arrTrigger = trigger_ID.split("|"); // to split the combined trigger_ID string
    for (var i = 0; i < arrTrigger.length; i++) {// to get the trigger element then add it to the trigger list
...
function CAgencyFly(element, settings, bIsIn) {
    ...
    for (var i = 0; i < arrAllSet.length; i++) {// to retrieve the setting
        ...
        switch (arrOneSet[0]) {
            case "speed" :
                this.duration = 100000 / arrOneSet[1];
                break;
            case "direction" :
                this.direction = arrOneSet[1];
                break;
        }
    }
...
CAgencyFly.prototype.ResetParameters = function () {
    ...
    if (this.bIsIn) { // FlyIn
        ...
        switch (this.direction) {
            case "right":
                this.startX = offsetRight;
                this.startY = this.finalY;
                break;
            case "left":
                this.startX = -offsetLeft;
                this.startY = this.finalY;
                break;
            case "down":
                this.startY = offsetBottom;
                this.startX = this.finalX;
                break;
            case "up":
                this.startY = -offsetTop;
                this.startX = this.finalX;
                break;
            case "downright":
        ...
        switch (this.direction) {
            case "right":
                this.finalX = offsetRight;
                this.finalY = this.startY;
                break;
            case "left":
                this.finalX = -offsetLeft;
                this.finalY = this.startY;
                break;
            case "down":
                this.finalY = offsetBottom;
                this.finalX = this.startX;
                break;
            case "up":
                this.finalY = -offsetTop;
                this.finalX = this.startX;
                break;
            case "downright":
                this.finalX = (offsetBottom < offsetRight) ? offsetBottom : offsetRight;
                this.finalY = this.finalX;
                break;
            case "upright":
...
CAgencyElastic.prototype.UpdateEffect = function () {
    ...
    if (percent >= 1.0)
        ...
        switch (this.direction) {
            case "left":
            case "right" :
                newX = rf * Math.cos(t) * rx + this.ele.ABSX;
                break;
            case "up":
            case "down" :
                newY = rf * Math.cos(t) * rx + this.ele.ABSX;
                break;
        }
        CCSSP.MoveObjectTo(this.ele, newX, newY);
        CCSSP.SetTimer(this, 20);
...
CAgencyElastic.prototype.ResetParameters = function () {
    ...
    switch (this.direction) {
        case "left":
            this.startX = -this.ele.offsetWidth;
            break;
        case "right":
            this.startX = this.ele.offsetWidth;
            break;
        case "up":
            this.startY = -this.ele.offsetHeight;
            break;
        case "down":
            this.startY = this.ele.offsetHeight;
            break;
...
function CAgencyAlpha(element, settings, bIsIn) {// because of "visual filter" style, this won't work in Navigator
    this.ele = element;
    this.bIsIn = bIsIn;
...
CAgencyAlpha.prototype.PrepareEffect = function () {// to set the visual filter function
...
CAgencyAlpha.prototype.UpdateEffect = function () {// to set the visual filter function
    if (this.timer == null)
        this.ResetParameters();
...
CAgencyAlpha.prototype.EndEffect = function () {// to remove the visual filter function
    clearInterval(this.timer);
    this.timer = null;
...
function CAgencyWave(element, settings) {// because of "visual filter" style, this won't work in Navigator
    this.ele = element;
    ...
    for (var i = 0; i < arrAllSet.length; i++) {// to retrieve the setting
        ...
        switch (arrOneSet[0]) {
            case "duration" :
                ...
                break;
            case "strength" :
                this.strength = arrOneSet[1];
                break;
            case "freq" :
                this.freq = arrOneSet[1];
                break;
            case "lightstrength" :
                this.lightstrength = arrOneSet[1];
                break;
        }
    }
...
CAgencyWave.prototype.PrepareEffect = function () {// to set the visual filter function
    CCSSP.PrepareFilter(this.ele);

    CCSSP.ShowObject(this.ele, true);
};

CAgencyWave.prototype.UpdateEffect = function () {// to set the visual filter function
    if (this.timer == null)
        this.ResetParameters();
...
CAgencyWave.prototype.EndEffect = function () {// to remove the visual filter function
    this.ele.style.filter = "";
    clearInterval(this.timer);
    this.timer = null;
};
...
function CAgencyGlow(element, settings) {// because of "visual filter" style, this won't work in Navigator
    this.ele = element;
    ...
    for (var i = 0; i < arrAllSet.length; i++) {
        ...
        switch (arrOneSet[0]) {
            case "color" :
                this.glowColor = arrOneSet[1];
                break;
            case "strength" :
                this.glowStrength = arrOneSet[1];
                break;
        }
    }
}
...
CAgencyGlow.prototype.PrepareEffect = function () {
    CCSSP.PrepareFilter(this.ele);
    CCSSP.ShowObject(this.ele, true);
    if (this.ele.style.backgroundColor != "") {//style.backgroundColor somehow stop the visual filter
        this.ele.intactBackgroundColor = this.ele.style.backgroundColor;
        this.ele.style.backgroundColor = "";
    }
};

CAgencyGlow.prototype.UpdateEffect = function () {// to set the visual filter function
    this.PrepareEffect();
    this.ele.style.filter = "glow(Color=" + this.glowColor + ", Strength=" +
        this.glowStrength + ", enabled=true" + ")";
};

CAgencyGlow.prototype.EndEffect = function () {// to remove the visual filter function
    this.ele.style.filter = "";
...
function CAgencyDropShadow(element, settings) {// because of "visual filter" style, this won't work in Navigator
    this.ele = element;
    ...
    for (var i = 0; i < arrAllSet.length; i++) {
        ...
        switch (arrOneSet[0]) {
            case "color" :
                this.shadowColor = arrOneSet[1];
                break;
            case "offx" :
                this.shadowOffx = arrOneSet[1];
                break;
            case "offy" :
                this.shadowOffy = arrOneSet[1];
                break;
        }
    }
}
...
CAgencyDropShadow.prototype.UpdateEffect = function () {// to set the visual filter function
    this.PrepareEffect();
    this.ele.style.filter = "dropshadow(color=" + this.shadowColor + ", offx=" +
        this.shadowOffx + ", offy=" + this.shadowOffy + ")";
};

CAgencyDropShadow.prototype.EndEffect = function () {// to remove the visual filter function
    this.ele.style.filter = "";
    if (typeof (this.ele.intactBackgroundColor) != "undefined")
        this.ele.style.backgroundColor = this.ele.intactBackgroundColor;
};
...
function CAgencyRevealTrans(element, settings) {// because of "visual filter" style, this won't work in Navigator
    this.ele = element;
    ...
    for (var i = 0; i < arrAllSet.length; i++) {
        ...
        switch (arrOneSet[0]) {
            case "type" :
                this.transition = arrOneSet[1];
                break;
            case "duration" :
                this.duration = 100 / arrOneSet[1];
                break;
        }
    }
}
...
CAgencyRevealTrans.prototype.UpdateEffect = function () {// to set the visual filter function
...
function CAgencyBlur(element, settings) {// because of "visual filter" style, this won't work in Navigator
    this.ele = element;
...
CAgencyBlur.prototype.UpdateEffect = function () {// to set the visual filter function
    CCSSP.PrepareFilter(this.ele);
    this.ele.style.filter = "blur(strength=" + this.strength +
        ", direction=" + this.direction + ")";
};

CAgencyBlur.prototype.EndEffect = function () {// to remove the visual filter function
    this.ele.style.filter = "";
};
...
CAgencyChangeFilter.prototype.UpdateEffect = function () {// to set the visual filter function
    CCSSP.PrepareFilter(this.ele);
    this.ele.style.filter = this.filterFunction;
};

CAgencyChangeFilter.prototype.EndEffect = function () {// to remove the visual filter function
    this.ele.style.filter = "";
};
...
function CAgencyFontChange(element, settings) {//this class can be replace by CAgencyChangeStyle,provided the "settings" is standard CSS string.
    this.ele = element;
    ...
    for (var i = 0; i < arrAllSet.length; i++) {// to retrieve the setting
        ...
        switch (arrOneSet[0]) {
            case "font-family" :
                this.newfontFamily = arrOneSet[1];
                break;
            case "color" :
                this.newfColor = arrOneSet[1];
                break;
            case "underline" :
                this.newtextDecoration = (arrOneSet[1] == "on") ? "underline" : "none";
                break;
            case "bold" :
                this.newfontWeight = (arrOneSet[1] == "on") ? "bold" : "normal";
                break;
            case "italic" :
                this.newfontStyle = (arrOneSet[1] == "on") ? "italic" : "normal";
                break;
            case "size" :
                this.newfontSize = arrOneSet[1];
                break;
            case "background-color" :
                this.newBackgroundColor = arrOneSet[1];
                break;
        }
    }
}
...
CAgencyFontChange.prototype.UpdateEffect = function () {// to change the font
    this.PrepareEffect();
...
CAgencyFontChange.prototype.EndEffect = function () {// to reinstate the original font style
    this.EndEffectAllChildren(this.ele);
    var arr = this.ele.getElementsByTagName('*');
    for (var index = 0; index < arr.length; index++)
        this.EndEffectAllChildren(arr[index]);
...
function CAgencyChangeStyle(element, settings) {//this class can be replace by CAgencyChangeStyle,provided the "settings" is standard CSS string.
    this.ele = element;
...
CAgencyChangeStyle.prototype.UpdateEffect = function () {// to change the style
    this.ele.style.cssText = this.newStyle;
};

CAgencyChangeStyle.prototype.EndEffect = function () {// to reinstate the original style
