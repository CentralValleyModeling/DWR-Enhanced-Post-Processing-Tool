...
def write_file(file_handle, compare_mode, name, refvar, data, dataIndex, title, series_name, yaxis, xaxis, plot_type,
               data_type, per_opt):
    print >> file_handle, """ {
    "output":"%s",
    "refvar":"%s",
    ...
    "yaxis_name": "%s %s",
    "xaxis_name": "%s", """ % (
        name, refvar, title, plot_type, data_type, series_name[0], series_name[1], series_name[2], per_opt, yaxis,
        xaxis)
...
def write_chart_data(file_handle, compare_mode, data):
    ...
    while i < n:
        ...
        if compare_mode == '5':
            print >> file_handle, "{x:%.0f,y0:%s,y1:%s,y2:%s}" % (
                x, format_for_nan(data[i][1]), format_for_nan(data[i][2]), format_for_nan(data[i][3]))
        if i != n - 1:
            print >> file_handle, ","
        i = i + 1
    print >> file_handle, "]"
