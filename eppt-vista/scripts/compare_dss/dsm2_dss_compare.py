...
def do_processing(globals, scalars, var_values, output_values, tw_values):
    ...
    for name in refname:
        ...
        try:
            ...
            continue
        if (globals['CALCULATE_SPECIFIED_RMSE_ONLY'] == 'OFF') or (
                globals['CALCULATE_SPECIFIED_RMSE_ONLY'] == 'ON' and name in output_values):
            conti = 0
            ...
            if conti == 0:
                ...
                if name in output_values:
                    ...
                    if intv < 1000:
                        ...
                        write_plot_data(fs_davg[cpart], compare_mode, name, str_refvar,
                                        compare_dss_utils.build_data_array(ref0_davg, ref1_davg, ref2_davg),
                                        dataIndex['spec_davg_' + cpart], "%s" % var_name, series_name,
                                        "%s (%s)" % (data_type, data_units), "Time", PlotType.TIME_SERIES, cpart,
                                        'Daily Average')
                        write_plot_data(fs_dmax[cpart], compare_mode, name, str_refvar,
                                        compare_dss_utils.build_data_array(ref0_dmax, ref1_dmax, ref2_dmax),
                                        dataIndex['spec_dmax_' + cpart], "%s" % var_name, series_name,
                                        "%s (%s)" % (data_type, data_units), "Time", PlotType.TIME_SERIES, cpart,
                                        'Daily Maximum')
                        write_plot_data(fs_dmin[cpart], compare_mode, name, str_refvar,
                                        compare_dss_utils.build_data_array(ref0_dmin, ref1_dmin, ref2_dmin),
                                        dataIndex['spec_dmin_' + cpart], "%s" % var_name, series_name,
                                        "%s (%s)" % (data_type, data_units), "Time", PlotType.TIME_SERIES, cpart,
                                        'Daily Minimum')
                    ...
                    if globals['PLOT_ORIGINAL_TIME_INTERVAL'] == 'ON':
                        ...
                        if dataIndex['spec_orig_' + cpart] > 1:
                            fs_orig[cpart].write(",")
                        write_plot_data(fs_orig[cpart], compare_mode, name, str_refvar,
                                        compare_dss_utils.build_data_array(ref0, ref1, ref2),
                                        dataIndex['spec_orig_' + cpart], "%s" % var_name, series_name,
                                        "%s (%s)" % (data_type, data_units), "Time", PlotType.TIME_SERIES, cpart,
                    ...
                    if intv < 40000:
                        ...
                        write_plot_data(fs_mavg[cpart], compare_mode, name, str_refvar,
                                        compare_dss_utils.build_data_array(ref0_mavg, ref1_mavg, ref2_mavg),
                                        dataIndex['spec_mavg_' + cpart], "%s" % var_name, series_name,
                                        "%s (%s)" % (data_type, data_units), "Time", PlotType.TIME_SERIES, cpart,
                                        'Monthly Average')
                ...
                if (globals['CALCULATE_SPECIFIED_RMSE_ONLY'] == 'OFF') or (
                    ...
                    if compare_mode == '3' or compare_mode == '4' or compare_mode == '5':
                        ...
                        for w in wy_types:
                            if compare_mode == '3':
                                diff_arr.append([vdiff.rmse_discrete_tws(ref1, ref2, wyt_arr[w], 0),
                                                 vdiff.rmse_discrete_tws(ref1, ref2, wyt_arr[w], 1)])
                            if compare_mode == '4':
                                diff_arr.append([vdiff.rmse_discrete_tws(ref0, ref1, wyt_arr[w], 0),
                                                 vdiff.rmse_discrete_tws(ref0, ref1, wyt_arr[w], 1)])
                            if compare_mode == '5':
                                diff_arr.append([vdiff.rmse_discrete_tws(ref0, ref1, wyt_arr[w], 0),
                                                 vdiff.rmse_discrete_tws(ref0, ref1, wyt_arr[w], 1),
                                                 vdiff.rmse_discrete_tws(ref0, ref2, wyt_arr[w], 0),
                                                 vdiff.rmse_discrete_tws(ref0, ref2, wyt_arr[w], 1)])
...
def write_summary_table(fh, dss_group1, dss_group2, globals, scalars, tw_values, var_values, output_values):
    compare_mode = globals['COMPARE_MODE']
    if compare_mode == '1': print >> fh, "<h1><center>Observation Time Series Report<br>Data Set: %s </center></h1>" % (
        scalars['NAME0'])
    if compare_mode == '2': print >> fh, "<h1><center>Model Output Time Series Report<br>Study Name: %s </center></h1>" % (
        scalars['NAME1'])
    if compare_mode == '3': print >> fh, "<h1><center>DSM2 Output Comparison Report<br> %s <n>vs</n> %s</center></h1>" % (
        scalars['NAME1'], scalars['NAME2'])
    if compare_mode == '4': print >> fh, "<h1><center>Calibration Report<br> %s <n>vs</n> %s</center></h1>" % (
        scalars['NAME1'], scalars['NAME0'])
    if compare_mode == '5': print >> fh, "<h1><center>Calibration Report<br>[ %s <n>vs</n> %s ] <n> & </n> [ %s <n>vs</n> %s ]</center></h1>" % (
        scalars['NAME1'], scalars['NAME0'], scalars['NAME2'], scalars['NAME0'])
...
"""
    for i in range(len(tw_values)):
        ...
        print >> fh, '<option value="%s" %s>%s</option>' % (
            compare_dss_utils.timewindow_option_value(vtimeseries.timewindow(tw_values[i][1].replace('"', ''))),
            selected,
            tw_values[i][0].replace('"', ''))
    print >> fh, """ 
</select> 
<div>
...
def write_plot_data(fh, compare_mode, name, refvar, data, dataIndex, title, series_name, yaxis, xaxis, plot_type,
                    data_type, per_opt):
    js_data.write_file(fh, compare_mode, name, refvar, data, dataIndex, title, series_name, yaxis, xaxis, plot_type,
                       data_type, per_opt);
...
"""
    ...
    if (globals['COMPARE_MODE'] != '1' and globals['COMPARE_MODE'] != '2'):
        if globals['COMPARE_MODE'] == '3': display_name = [scalars['NAME2'], scalars['NAME1'], scalars['NAME2'],
                                                           scalars['NAME1']]
        if globals['COMPARE_MODE'] == '4': display_name = [scalars['NAME1'], scalars['NAME0'], scalars['NAME1'],
                                                           scalars['NAME0']]
        if globals['COMPARE_MODE'] == '5': display_name = [scalars['NAME1'] + '/' + scalars['NAME2'], scalars['NAME0'],
                                                           scalars['NAME1'] + '/' + scalars['NAME2'], scalars['NAME0']]
        print >> fh, """
    ...
    """ % (display_name[0], display_name[1], display_name[2], display_name[3])
       ...
       """
        if (globals['COMPARE_MODE'] == '5'):
         ...
         tbl_sel[z]+='</tr>';
        """ % (
                scalars['NAME0'], scalars['NAME1'], scalars['NAME0'], scalars['NAME2'], scalars['NAME0'],
                scalars['NAME1'],
                scalars['NAME0'], scalars['NAME2'])
        print >> fh, """
