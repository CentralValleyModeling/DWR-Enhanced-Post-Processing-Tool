    ...
    for i in str_arr:
        if i.find(" ") >= 0:
            logging.warning(
                "*******************************************************************************************************************")
            logging.warning(
                "Time series with C PART '" + i + "' will not be displayed. Please replace space in PART C with other valid characters.")
            logging.warning(
                "*******************************************************************************************************************")
...
def get_group_ref(globals, scalars, var_values):
    ...
    for name in refnam:
        try:
            if (compare_mode == '1' and refvar[name][0] == 'NA') or \
                    ...
                    (compare_mode == '5' and (
                            refvar[name][0] == 'NA' or refvar[name][1] == 'NA' or refvar[name][2] == 'NA')):
                del (refvar[name])
                refnam.remove(name)
        except Exception, e:
            refnam.remove(name)
    ...
    for i in range(nvar_values):
        refvar[var_values[i][0].encode('ascii')] = [var_values[i][1].encode('ascii'), var_values[i][2].encode('ascii'),
                                                    var_values[i][3].encode('ascii')]
...
def get_series_str(scalars, refvar):
    ...
    for i in range(3):
        if refvar[i] != 'NA':
            series_arr.append(get_filename_from_refvar(refvar[i], scalars) + "::" + get_bpart_from_refvar(
                refvar[i]) + "_" + get_cpart_from_refvar(refvar[i]))
        else:
...
def get_month(value):
    valueDic = {"JAN": "01", "FEB": "02", "MAR": "03", "APR": "04", "MAY": "05", "JUN": "06", "JUL": "07", "AUG": "08",
