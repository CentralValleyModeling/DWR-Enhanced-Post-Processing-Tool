...
def prepCondorDir(perturbed):
    ...
    / v / b % HYDROEXE % condorDir \
      @ copy / v / b % QUALEXE % condorDir \
      @ xcopy / i / q % TSDIR % condorDir

    @xcopy

    / i / q % COMMONDIR % condorDir

    @copy

    / v / y % CONFIGFILE % condorDir \
      @ copy / v / y
    dsm2 - base.sub
    condorDir \
    @ copy / v / y % SENSDIR %\BaseRun - 0\ % DSM2RUN %.?rf
    condorDir \
    @ copy / v / y
    hydro_restart.inp
    condorDir\hydro.inp
    ...
    / v / y
    qual_ec_restart.inp
    condorDir\qual_ec.inp
    set
    START_DATE = "01OCT1992"
    ...
    echo
    del / f / q *.h5 *.??f >> condor_dsm2.bat
    echo
    del / f / q * echo *.inp >> condor_dsm2.bat
    ...
    for / F usebackq % % F in (`dir / b / o:n % FLIST % `) do
    call:Condor % % F
    rem
    Next
    line
    ...
    / y % SENSDIR %\ % PTBDIR %\PerturbedInp % PRTBPREFIX % % 1.
    inp.
    ...
    don
    't
    echo % PRTBPREFIX % | c:\windows\system32\find.exe / i
    "DICU"
    if errorlevel 1 (
    ...
    echo % PRTBPREFIX % | c: \
            windows\system32\find.exe / i
    "ECRET"
    if errorlevel
    1(
        set
    BLOCKTYPE = HYDRO_TIME_SERIES
    ) else (
    ...
    echo
    CONFIGURATION > tempEnv.inp
    echo % 1 >> tempEnv.inp
    echo
    config.inp >> tempEnv.inp
    echo
    END >> tempEnv.inp

    echo % BLOCKTYPE % > tempInp.inp
    echo
    PerturbedInp % PRTBPREFIX % % 1.
    inp >> tempInp.inp
    echo
    END >> tempInp.inp
