...
TEST_MAP_BOUNDS = {"LatNorth": 38.1969102804601, "LatSouth": 38.08836429508079, "LonWest": -121.69795989990236,
                   "LonEast": -121.49127960205078}
...
def _get_station_ids(bounds):
    ...
    argumentCollection = '{"LatNorth":%f,"LatSouth":%f,"LonWest":%f,"LonEast":%f,"getGW":false,"getWQ":true,"getHY":false,"getUser":"TRUE"}' % (
        bounds['LatNorth'], bounds['LatSouth'], bounds['LonWest'], bounds['LonEast'])
...
def get_station_ids_from_county(county):
    ...
    return re.findall(re.compile(
        '<td.*?>.*?value=".*?".*?</td>.*?<td.*?>.*?</td>.*?<td.*?>(.*?)</td>.*?<td.*?>.*?</td>.*?<td.*?>.*?</td>',
        re.S | re.M | re.I), result)
...
def get_data_for_station(station_id, from_map, sdate, edate):
    ...
    result = reduce(lambda x, y: x + y, result)
    boxstation_id = \
        re.findall(re.compile('<input.*? name="boxStationID".*? value="(.*?)" .*?>', re.S | re.M | re.I), result)[0]
    if len(sdate) < 10:
    ...
    response = urllib.urlopen(
        'http://www.water.ca.gov/waterdatalibrary/waterquality/station_county/report_content_xcl_txt.cfm', params)
