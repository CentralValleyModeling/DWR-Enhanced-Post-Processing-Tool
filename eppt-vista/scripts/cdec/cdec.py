...
class Sensor:
    ...
    def printAll(self):
        print 'Sensor %s: %s is of type %s/%s with units %s and duration of %s' % (
            self.id, self.sensor_number, self.type, self.subType, self.duration, self.units)
...
def retrieve_station_sensors(station):
    data = fetch_data_in_url(
        "/cgi-progs/queryCSV?station_id=%s&sensor_num=&dur_code=D&start_date=&end_date=&data_wish=View+CSV+Data" % station.id)
        ...
        if div.find('Sensor numbers available') > 0:
            ...
            for row in rows:
                ...
                if sensor:
                    ...
                    print 'No sensor found for type: %s, subtype: %s, units: %s, duration: %s' % (
                        type, subType, units, duration)
...
def retrieve_csv(station, sensor, timewindow):
    ...
    url = "/cgi-progs/queryCSV?station_id=%s&dur_code=%s&sensor_num=%s&start_date=%s&end_date=%s" % (
        station.id, sensor.getDurationCode(), sensor.sensor_number, start_date, end_date)
    print url
...
def retrieve_ts(station, sensor, \
    ...
    pathname = '/%s/%s/%s//%s/%s/' % (
        'CDEC-RAW', station_name, c_part, DSS_INTERVAL[sensor.duration], "SENSOR " + sensor.id)
    units = sensor.units
...
def retrieve(station_name, sensor_number, start_date, end_date='now', verbose=1):
    ...
    if not sensor:
        ...
        for sensor in station.sensors:
            print 'SensorId: %s, Sensor Number: %s, Type: %s, Sub Type: %s, Data Available: %s' % (
                sensor.id, sensor.sensor_number, sensor.type, sensor.subType, sensor.dataAvailable)
            return None
