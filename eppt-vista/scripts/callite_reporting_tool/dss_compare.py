    ...
    */""" % (str(Date()))
    ...
    for path_map in pathname_maps:
        ...
        if path_map.report_type == 'Average':
            write_plot_data(fh, build_data_array(ref1, ref2, tw), dataIndex,
                            "Average %s" % path_map.var_name.replace('"', ''), series_name,
                            "%s(%s)" % (data_type, data_units), "Time", PlotType.TIME_SERIES)
        elif path_map.report_type == 'Exceedance':
            write_plot_data(fh, build_exceedance_array(ref1, ref2, path_map.var_category == 'S_SEPT', tw), dataIndex,
                            get_exceedance_plot_title(path_map), series_name, "%s(%s)" % (data_type, data_units),
            ...
            write_plot_data(fh, build_exceedance_array(ref1, ref2, tw), dataIndex,
                            "Exceedance %s" % path_map.var_name.replace('"', ''), series_name,
                            "%s(%s)" % (data_type, data_units), "Percent at or above", PlotType.EXCEEDANCE)
...
"""
    for i in range(len(tw_values)):
        ...
        print >> fh, '<option value="%s" %s>%s</option>' % (
            timewindow_option_value(vtimeseries.timewindow(tw_values[i][1].replace('"', ''))), selected,
...
def write_water_balance_table(fh, dss_group1, dss_group2, scalars, pathname_maps, tw_values):
    ...
    for tw in tws:
        print >> fh, '<td>%s</td><td>%s</td><td>Diff</td><td>%% Diff</td>' % (
            (scalars['NAME2'] + "<br/>Alt", scalars['NAME1'] + "<br/>Base"))
    print >> fh, "</tr>"
    index = 0
    for path_map in pathname_maps:
        var_name = path_map.var_name
