...
class InputPanel(JPanel):
    def __init__(self):
        ...
        self.fileobs = JTextField(prefs.get("file.obs", "Z:/condor_jobs_cmaes/output_obs/Observed_data_all_forViz.dss"),
                                  25)
        self.filerun1 = JTextField(
            prefs.get("file.run1", "Z:/condor_jobs_cmaes/output_obs/previous_run_results/hist_mini_calib_v811_317.dss"),
            25)
        self.filerun2 = JTextField(
            prefs.get("file.run2", "Z:/condor_jobs_cmaes/output_test/hist_mini_calib_v811_0.dss"), 25)
        self.image_dir = JTextField(prefs.get("image.dir", "d:/temp"), 25)
        self.locations = JTextField(
            prefs.get("locations", "RSAC077,RSAN007,RSAC081,RSAC092,RSAN018,RSAN032,ROLD024,CLIFTON_COURT"), 35)
    ...
    def dorun(self, event):
        doall(str.split(str(self.locations.text), ","), str(self.fileobs.text), str(self.filerun1.text),
              str(self.filerun2.text), str(self.stime.text), str(self.etime.text), str(self.image_dir.text), None)
...
def doall(locations, fileobs, filerun1, filerun2, stime, etime, imageDir='d:/temp', weights=None, filter_type="AVE",
          normalize=False):
    ...
    for l in locations:
        ...
        legend_label.setText(legend_label.getText() + " [" + str(int(lrms1 * 100) / 100.) + "," + str(
            int(lrms1_min * 100) / 100.) + "," + str(int(lrms1_max * 100) / 100.) + "]")
        legend_label = plotd.getLegendLabel(drun2d.data)
        legend_label.setText(legend_label.getText() + " [" + str(int(lrms2 * 100) / 100.) + "," + str(
            int(lrms2_min * 100) / 100.) + "," + str(int(lrms2_max * 100) / 100.) + "]")
